...
export const deleteStudentSubjectRecord = async(req,res)=>{
    try{
        ...
        const record= await StudentSubject.findOneAndDelete({student:studentId, subject:subjectId})
        if(!record){
            return res.status(404).json("StudentSubject record not found")
        }
...
export const getSubjectsByStudent = async(req,res)=>{
    try{
        ...
        const records= await StudentSubject.find({student:studentId}).populate('subject')
        res.status(200).json(records)
    }
    catch(error){
        res.status(500).json({message:error.message})
...
export const getStudentsBySubject = async(req,res)=>{
    try{
        ...
        const records= await StudentSubject.find({subject:subjectId}).populate('student')
        res.status(200).json(records)
    }
    catch(error){
        res.status(500).json({message:error.message})
