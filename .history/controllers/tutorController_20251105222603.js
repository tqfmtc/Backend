...
export const createTutor = async (req, res) => {
  try {
    ...
    if (!Array.isArray(subjects)) subjectsArray = subjects ? [subjects] : [];

    // NEW: normalize students
    let studentsArray = normalizeIdArray(students) || [];
    // Validate ObjectIds
    for (const sid of studentsArray) {
      if (!isValidObjectId(sid)) return res.status(400).json({ message: `Invalid student id: ${sid}` });
    }
...
export const updateTutor = async (req, res) => {
  try {
    ...
    // NEW: Students
    // Support three modes:
    // 1) Replace array: students: [ids]
    // 2) Add some: studentsAdd: [ids]
    // 3) Remove some: studentsRemove: [ids]
    ...
    for (const sid of toValidate) {
      if (!isValidObjectId(sid)) {
        return res.status(400).json({ message: `Invalid student id: ${sid}` });
      }
    }

    // If replace provided, set it; otherwise compute after save with $addToSet/$pull
    if (studentsReplace) {
      updateData.students = studentsReplace;
    }
    ...
    // Documents (as in your existing code)
    ...
    // Apply students add/remove if present and not replaced
    if (!studentsReplace && (studentsAdd?.length || studentsRemove?.length)) {
      const updateOps = {};
      if (studentsAdd?.length) updateOps.$addToSet = { students: { $each: studentsAdd } };
      if (studentsRemove?.length) updateOps.$pull = { students: { $in: studentsRemove } };
      if (Object.keys(updateOps).length) {
        await Tutor.updateOne({ _id: tutor._id }, updateOps);
      }
    }

    // Return updated tutor with populated students (optional)
    const updated = await Tutor.findById(tutor._id)
      .populate('students', '_id name phone') // adjust fields as your Student model has
      .lean();
